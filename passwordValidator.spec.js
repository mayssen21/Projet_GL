...
test("le mot de passe doit contenir au moins 8 caractères", () => {
  ...
  expect(isPasswordValid("1234567", { minLength: 8, minLetters: 1, minNumbers: 1 })).toBe(false); 
...
test("mot de passe avec règle personnalisée : caractère spécial", () => {
  const specialCharRule = pwd => /[!@#$%^&*]/.test(pwd);
   expect(isPasswordValid("abc123!", { minLength: 6, minLetters: 1, minNumbers: 1, newRules: [specialCharRule] })).toBe(true);
   expect(isPasswordValid("abc123", { minLength: 6, minLetters: 1, minNumbers: 1, newRules: [specialCharRule] })).toBe(false);
});